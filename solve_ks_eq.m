...
function [ret_ee,ret_ev] = solve_ks_eq(NPTS,NSTM,a,b,V,k)
...
z=1:NPTS; % indexes for main diagonal
L=b-a;
...
if fourth_order == false
    ...
    Ham = zeros(NPTS,NPTS);
    HamKS1 = zeros(NPTS,NPTS);
    HamKS2 = zeros(NPTS,NPTS);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Construction of second Hamiltonian for k-points sampling -k^2*I
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
HamKS1 = -(k*k)*eye(NPTS);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Begin construction on third Hamiltonian for k-point sampling
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 

HamKS2(1, 2) = (2*j*k) / (2*h);
HamKS2(1, NPTS) = -(2*j*k) / (2*h);
HamKS2(NPTS, 1) = (2*j*k) / (2*h);
HamKS2(NPTS, NPTS-1) = -(2*j*k) / (2*h);

 for i=2:NPTS-1
 
     HamKS2(i , i+1) = (2*j*k) / (2*h);
     HamKS2(i, i-1) = -(2*j*k) / (2*h);
        
 end
 HamKS2;
 %Above is printing out matrix to make sure entered correctly
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Now multiply by 2ik
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Combine three hamiltonians then get eigenvalues + eigenvector
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
HamFinal = Ham + HamKS1 + HamKS2;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
